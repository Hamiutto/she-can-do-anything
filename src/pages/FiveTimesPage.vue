...
<template>
  <section class="page-section section is-active five-times-page">
    <div class="page-card five-times-card">
      ...
      <h2>五种时间 收获复利人生</h2>
      <p class="lead">
        把时间看成可以被感知、分配和复利的生命资产。先了解五种时间的定义，再往下看对应的人生样本。
      </p>
    ...
    <section ref="timeResultsRef" class="page-card time-results">
      <div class="section-title">
        <div>
          <h2>{{ selectedTimeInfo ? `${selectedTimeInfo.title} 的人生模板` : '选择一个时间' }}</h2>
          <p>
            {{
              selectedTimeInfo
                ? `当前展示 ${matchingPeople.length} 位带有「${selectedTimeInfo.tag}」标签的样本卡。`
                : '点击上方时间卡片，页面会在这里展开对应样本。'
            }}
          </p>
        </div>
        <div class="filters">
          <button v-if="selectedTime" class="soft-button" type="button" @click="clearTime">清除时间</button>
        </div>
      </div>
